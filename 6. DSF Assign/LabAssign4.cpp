...
public:
    ...
    int Delete()
    {
        int x = data[F]; // R will increase while inserting F remain same PRN: 22010666 if (R == F)
        if(R==F)
            R = F = -1;
        else
            F = F + 1;
...
void graph::readgraph()
{
    ...
    cin >> no_of_edges;
    // data of node
    // F will increase while deleting nodefor (i = 0; i < no_of_edges; i++)
    {
        cout << "\nEnter an edge (u,v) : ";
        cin >> vi >> vj;
        insert(vi, vj);
...
void graph::DFS1(int i) // i is the sourse node
{
    ...
    cout << "\n"
         << i << " ";
    p = G[i];
    // storing sourse node in P
    visited[i] = 1;
    // just mark it visited
    while (p != NULL) //
    {
